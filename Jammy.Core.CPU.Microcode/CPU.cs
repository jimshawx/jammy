...
#pragma warning disable IDE1006 // Naming Styles
		...
		private static readonly uint[] d = new uint[8];
		private static A a;
		...
		//T.S..210...XNZVC
		private static ushort sr;

		private static uint instructionStartPC;
		private static ushort undisturbedSR;

		private static FetchMode fetchMode;

		private static IInterrupt interrupt;
		private static ITracer tracer;
		private static EmulationSettings settings;
		private static ILogger logger;

		private static IMemoryMapper memoryMapper;
		private static IBreakpointCollection breakpoints;

		public static void CPUInit(IInterrupt interrupt, IMemoryMapper memoryMapper,
			IBreakpointCollection breakpoints, ITracer tracer,
			IOptions<EmulationSettings> settings,
			ILogger logger)
		{
			CPU.a = new A(Supervisor);

			CPU.interrupt = interrupt;
			CPU.memoryMapper = memoryMapper;
			CPU.breakpoints = breakpoints;
			CPU.tracer = tracer;
			CPU.settings = settings.Value;
			CPU.logger = logger;
		...
		private static Regs gregs = new Regs();
		private static ushort Fetch(uint pc)
		{
			tracer.TraceAsm(pc, GetRegs(gregs));
			return fetch16(pc);
		}

		private static bool CheckInterrupt()
		{
		...
		public static Regs GetRegs(Regs regs)
		{
			for (int i = 0; i < 8; i++)
			{
				regs.A[i] = a[i];
		...
		public static void SetRegs(Regs regs)
		{
			for (int i = 0; i < 8; i++)
			{
				a[i] = regs.A[i];
		...
		public static void SetPC(uint pc)
		{
			CPU.pc = pc;
		}
		...
		private static void clrSupervisor()
		{
		...
		private static void setX(bool val)
		{
			if (val) setX(); else clrX();
		}

		private static void setX(uint val)
		{
		...
		private static void clrN()
		{
			sr &= 0b1111111111110111;
		}

		private static void setN(bool val)
		{
		...
		private static void clrZ()
		{
			sr &= 0b11111111_11111011;
		}

		private static void setZ(bool val)
		{
		...
		private static void clrV()
		{
			sr &= 0b11111111_11111101;
		}

		private static void setV(long val, Size size)
		{
			if (size == Size.Long)
		...
		private static void setV(bool val)
		{
		...
		private static void clrC()
		{
			sr &= 0b11111111_11111110;
		}

		private static void setC(bool val)
		{
			if (val) setC(); else clrC();
		}

		private static void setC(uint val)
		{
			if (val != 0) setC(); else clrC();
		}

		private static void setC(ulong val)
		{
			if (val != 0) setC(); else clrC();
		}

		private static void setC(long val)
		{
			if (val != 0) setC(); else clrC();
		}

		private static void setC(ulong val, Size size)
		{
			if (size == Size.Long)
				setC(val > 0xffffffff);
			else if (size == Size.Word)
		...
		private static void setNZ(uint val, Size size)
		{
			int c;
		...
		private static void setN(uint val, Size size)
		{
			int c;
		...
		private static void setZ(uint val, Size size)
		{
			int c;
		...
		private static void setC(uint v, uint op, Size size)
		{
			ulong r = (ulong)zeroExtend(v, size) + (ulong)zeroExtend(op, size);
			setC(r, size);
		}

		private static void setC_sub(uint v, uint op, Size size)
		{
			ulong r = zeroExtend(v, size) - zeroExtend(op, size);
			setC(r, size);
		}

		private static void setC_subx(uint v, uint op, Size size)
		{
			ulong r = zeroExtend(v, size) - zeroExtend(op, size) - (X()?1ul:0ul);
			setC(r, size);
		}

		private static void setC_addx(uint v, uint op, Size size)
		{
			ulong r = (ulong)zeroExtend(v, size) + (ulong)zeroExtend(op, size) + (X()?1UL:0UL);
			setC(r, size);
		}

		private static void setV(uint v, uint op, Size size)
		{
			long r = signExtend(v, size) + signExtend(op, size);
			setV(r, size);
		}

		private static void setV_sub(uint v, uint op, Size size)
		{
			long r = signExtend(v, size) - signExtend(op, size);
			setV(r, size);
		}

		private static void setV_subx(uint v, uint op, Size size)
		{
			long r = signExtend(v, size) - signExtend(op, size) - (X() ? 1L : 0L);
			setV(r, size);
		}

		private static void setV_addx(uint v, uint op, Size size)
		{
			long r = signExtend(v, size) + signExtend(op, size) + (X() ? 1L : 0L);
			setV(r, size);
		}

		private static void clrCV()
		{
			clrC();
			clrV();
		}

		private static bool X() { return (sr & 16) != 0; }
		private static bool N() { return (sr & 8) != 0; }
		private static bool Z() { return (sr & 4) != 0; }
		private static bool V() { return (sr & 2) != 0; }
		private static bool C() { return (sr & 1) != 0; }
		private static bool Supervisor() { return (sr & 0b00100000_00000000) != 0; }
		...
		public static uint fetch32(uint address)
		{
		...
		public static ushort fetch16(uint address)
		{
		...
		private static void push32(uint value)
		{
			a[7] -= 4;
			write32(a[7], value);
		}

		private static void push16(ushort value)
		{
			a[7] -= 2;
			write16(a[7], value);
		}

		private static uint pop32()
		{
			uint val = read32(a[7]);
			a[7] += 4;
			return val;
		...
		private static uint fetchEA(int type, Size size)
		{
			int m = (type >> 3) & 7;
			int x = type & 7;
		...
		private static uint fetchOpSize(uint ea, Size size)
		{
		...
		private static uint fetchImm(Size size)
		{
			uint v = 0;
			if (size == Size.Long)
			{
		...
		private static uint fetchOp(int type, uint ea, Size size)
		{
		...
		private static void writeOp(uint ea, uint val, Size size)
		{
			//todo: trap on odd aligned access
			if (size == Size.Long)
		...
		private static void writeEA(int type, uint ea, Size size, uint value)
		{
			int m = (type >> 3) & 7;
			int Xn = type & 7;
		...
		private static void t_fourteen(int type)
		{
		...
		private static void rod(int type, int rot, int lr, Size size)
		{
		...
		private static void roxd(int type, int rot, int lr, Size size)
		{
			uint ea = fetchEA(type, size);
			uint val = fetchOp(type, ea, size);
			val = (uint)zeroExtend(val, size);
		...
		private static void lsd(int type, int shift, int lr, Size size)
		{
			uint ea = fetchEA(type, size);
		...
		private static void asd(int type, int shift, int lr, Size size)
		{
			uint ea = fetchEA(type, size);
		...
		private static void t_thirteen(int type)
		{
			//add
		...
		private static void t_twelve(int type)
		{
		...
		private static void and(int type)
		{
			Size size = getSize(type);
			uint ea = fetchEA(type, size);
			uint op = fetchOp(type, ea, size);
		...
		private static void exg(int type)
		{
			int Yn = type & 7;
			int Xn = (type >> 9) & 7;
		...
		private static void abcd(int type)
		{
			//only Dx,Dy and -(Ax),-(Ay) allowed
			if ((type & 0b1_000) != 0) type ^= 0b101_000;
		...
		private static uint add_bcd(uint dst, uint src)
		{
		...
		private static uint add_bcd2(uint op, uint op2)
		{
			byte c= (byte)(X() ? 1 : 0);
			byte d0;
			byte d1;
		...
		private static void muls(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void mulu(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void t_eleven(int type)
		{
		...
		private static void eor(int type)
		{
			Size size = getSize(type);
			uint ea = fetchEA(type, size);
			uint op = fetchOp(type, ea, size);
		...
		private static void cmpm(int type)
		{
			Size size = getSize(type);
		...
		private static void cmp(int type)
		{
			Size size = getSize(type);
			uint ea = fetchEA(type, size);
		...
		private static void cmpa(int type)
		{
			Size size;
			if ((type & 0b1_00_000_000) != 0)
				size = Size.Long;
		...
		private static void t_nine(int type)
		{
			//sub
		...
		private static void t_eight(int type)
		{
		...
		private static void or(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void sbcd(int type)
		{
			//only Dx,Dy and -(Ax),-(Ay) allowed
			if ((type & 0b1_000) != 0) type ^= 0b101_000;
		...
		private static uint sub_bcd(uint dst, uint src)
		{
		...
		private static uint sub_bcd2(uint op, uint op2)
		{
			//op-op2
			sbyte c = (sbyte)(X() ? 1 : 0);
			sbyte d0;
		...
		private static void divs(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void divu(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void t_seven(int type)
		{
			if (((type >> 8) & 1) == 0)
			{
				//moveq
		...
		private static void t_six(int type)
		{
			uint target;
		...
		//3-19
		private static bool le()
		{
			return Z() || (N() && !V()) || (!N() && V());
		}

		private static bool gt()
		{
			return (N() && V() && !Z()) || (!N() && !V() && !Z());
		}
		...
		private static bool vs()
		{
			return V();
		}

		private static bool vc()
		{
			return !V();
		}
		...
		private static void bsr(int type, uint target)
		{
		...
		private static void bra(int type, uint target)
		{
		...
		private static void t_five(int type)
		{
		...
		private static void scc(int type)
		{
			uint ea = fetchEA(type, Size.Byte);
			int cond = (type >> 8) & 0xf;
			switch (cond)
		...
		private static void dbcc(int type)
		{
			void dec16(int Xn)
			{
		...
		private static void subq(int type, Size size)
		{
		...
		private static void addq(int type, Size size)
		{
			uint imm = (uint)((type >> 9) & 7);
			uint ea = fetchEA(type, size);
			uint op = fetchOp(type, ea, size);
		...
		private static void t_four(int type)
		{
		...
		private static void tst(int type)
		{
		...
		private static void tas(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void not(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void neg(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void clr(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void negx(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void pea(int type)
		{
		...
		private static void swap(int type)
		{
			int Xn = type & 7;
		...
		private static void nbcd(int type)
		{
			uint ea = fetchEA(type, Size.Byte);
			uint op = fetchOp(type, ea, Size.Byte);
			op &= 0xff;
		...
		private static void ext(int type)
		{
			int Xn = type & 7;
			int mode = (type >> 6) & 7;
			switch (mode)
		...
		private static void movetosr(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void movetoccr(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void movefromsr(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void illegal(int type)
		{
			internalTrap(4);
		}

		private static readonly string[] trapNames = new string[16] {
			"Initial SSP",
			"Initial PC",
			"Bus Error",
			"Address Error",
		...
		private static void internalTrap(uint vector)
		{
			ushort oldSR = sr;
		...
		private static void trap(int type)
		{
		...
		private static void link(int type)
		{
			int An = type & 7;
		...
		private static void unlk(int type)
		{
			int An = type & 7;
			a[7] = a[An];
			a[An] = pop32();
		...
		private static void moveusp(int type)
		{
			if (Supervisor())
			{
				int An = type & 7;
		...
		private static void nop(int type)
		{
		}

		private static void stop(int type)
		{
			if (Supervisor())
			{
		...
		private static void t_zero(int type)
		{
		...
		private static void movep(int type)
		{
			Size size;
			int Xn = (type>>9) & 7;
		...
		private static void movel(int type)
		{
		...
		private static void movew(int type)
		{
			uint ea = fetchEA(type, Size.Word);
			uint op = fetchOp(type, ea, Size.Word);
			type = swizzle(type);
		...
		private static void moveb(int type)
		{
			if(IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void cmpi(int type)
		{
		...
		private static void eori(int type)
		{
			Size size = getSize(type);
			uint imm = fetchImm(size);
		...
		private static void bit(int type)
		{
			Size size;
		...
		private static void addi(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void subi(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void andi(int type)
		{
			Size size = getSize(type);
			uint imm = fetchImm(size);
		...
		private static void ori(int type)
		{
			Size size = getSize(type);
			uint imm = fetchImm(size);
		...
		private static void chk(int type)
		{
			if (IsAddressReg(type))
			{
				internalTrap(3);
		...
		private static void lea(int type)
		{
			//some EA are not valid
			if (IsAddressReg(type) || IsDataReg(type) || IsPostIncrement(type) || IsPreDecrement(type) || IsImmediate(type))
			{
		...
		private static void movem(int type)
		{
		...
		private static void jmp(int type)
		{
		...
		private static void jsr(int type)
		{
		...
		private static void rtr(int type)
		{
		...
		private static void trapv(int type)
		{
			if (V())
				internalTrap(7);
		}

		private static void rts(int type)
		{
		...
		private static void rte(int type)
		{
			if (Supervisor())
			{
