...
static int32 roundAndPackInt32( flag zSign, bits64 absZ )
{
	...
	if ( roundBits ) float_exception_flags |= float_flag.float_flag_inexact;
	return z;
...
static int64 roundAndPackInt64( flag zSign, bits64 absZ0, bits64 absZ1 )
{
	...
	if ( ! roundNearestEven ) {
		if ( roundingMode == float_round.float_round_to_zero) {
			...
			if ( zSign ) {
				increment = ( roundingMode == float_round.float_round_down) && absZ1;
			}
			else {
				increment = ( roundingMode == float_round.float_round_up) && absZ1;
			}
		}
	}
	if ( increment ) {
	...
	if ( absZ1 ) float_exception_flags |= float_flag.float_flag_inexact;
	return z;
...
static float32 roundAndPackFloat32( flag zSign, int16 zExp, bits32 zSig )
{
	...
	if ( 0xFD <= (bits16) zExp ) {
		...
		if ( zExp < 0 ) {
			...
			shift32RightJamming( zSig, - zExp, out zSig );
			zExp = 0;
			roundBits = zSig & 0x7F;
			if ( isTiny && roundBits ) float_raise( float_flag.float_flag_underflow );
		}
	}
	if ( roundBits ) float_exception_flags |= float_flag.float_flag_inexact;
...
static float64 roundAndPackFloat64( flag zSign, int16 zExp, bits64 zSig )
{
	...
	if ( 0x7FD <= (bits16) zExp ) {
		...
		if ( zExp < 0 ) {
			...
			shift64RightJamming( zSig, - zExp, out zSig );
			zExp = 0;
			roundBits = zSig & 0x3FF;
			if ( isTiny && roundBits ) float_raise( float_flag.float_flag_underflow );
		}
	...
	)
{
	...
	if ( 0x7FFD <= (bits32) ( zExp - 1 ) ) {
		...
		if ( zExp <= 0 ) {
			...
			shift64RightJamming( zSig0, 1 - zExp, out zSig0 );
			zExp = 0;
			roundBits = zSig0 & roundMask;
			if ( isTiny && roundBits ) float_raise( float_flag.float_flag_underflow );
			if ( roundBits ) float_exception_flags |= float_flag.float_flag_inexact;
	...
	if ( ! roundNearestEven ) {
		if ( roundingMode == float_round.float_round_to_zero ) {
			...
			if ( zSign ) {
				increment = ( roundingMode == float_round.float_round_down ) && zSig1;
			}
			else {
				increment = ( roundingMode == float_round.float_round_up ) && zSig1;
			}
		}
	}
	if ( 0x7FFD <= (bits32) ( zExp - 1 ) ) {
	...
	overflow:
		...
		if ( zExp <= 0 ) {
			isTiny =
				...
				|| ! increment
				|| ( zSig0 < LIT64( 0xFFFFFFFFFFFFFFFF ) );
			shift64ExtraRightJamming( zSig0, zSig1, 1 - zExp, out zSig0, out zSig1 );
			zExp = 0;
			if ( isTiny && zSig1 ) float_raise(float_flag.float_flag_underflow);
			if ( zSig1 ) float_exception_flags |= float_flag.float_flag_inexact;
			if ( roundNearestEven ) {
				increment = ( (sbits64) zSig1 < 0 );
			}
			else {
...
//    bits32 zSig;
	...
	if ( 0 <= shiftCount ) {
		...
		shiftCount += 7;
		if ( shiftCount < 0 ) {
			shift64RightJamming( absA, - shiftCount, out absA );
		}
		else {
			absA <<= shiftCount;
		}
...
static float32 addFloat32Sigs( float32 a, float32 b, flag zSign )
{
	...
	if ( 0 < expDiff ) {
		...
		shift32RightJamming( aSig, - expDiff, out aSig );
		zExp = bExp;
	}
	else {
		if ( aExp == 0xFF ) {
...
static float64 addFloat64Sigs( float64 a, float64 b, flag zSign )
{
	...
	if ( 0 < expDiff ) {
		...
		shift64RightJamming( aSig, - expDiff, out aSig );
		zExp = bExp;
	}
	else {
		if ( aExp == 0x7FF ) {
...
static int32 floatx80_to_int32( floatx80 a )
{
	...
	shift64RightJamming( aSig, shiftCount, out aSig );
	return roundAndPackInt32( aSign, aSig );
...
static floatx80 addFloatx80Sigs( floatx80 a, floatx80 b, flag zSign )
{
	...
	if ( 0 < expDiff ) {
		...
		if ( bExp == 0 ) --expDiff;
		shift64ExtraRightJamming( bSig, 0, expDiff, out bSig, out zSig1 );
		zExp = aExp;
	}
	else if ( expDiff < 0 ) {
		if ( bExp == 0x7FFF ) {
		...
		if ( aExp == 0 ) ++expDiff;
		shift64ExtraRightJamming( aSig, 0, - expDiff, out aSig, out zSig1 );
		zExp = bExp;
	}
	else {
		if ( aExp == 0x7FFF ) {
...
static floatx80 subFloatx80Sigs( floatx80 a, floatx80 b, flag zSign )
{
	...
	if ( aExp == 0 ) ++expDiff;
	shift128RightJamming( aSig, 0, - expDiff, out aSig, out zSig1 );
	bBigger:
	...
	if ( bExp == 0 ) --expDiff;
	shift128RightJamming( bSig, 0, expDiff, out bSig, out zSig1 );
	aBigger:
...
//    bSign = extractFloatx80Sign( b );
	...
	if ( 0 < expDiff ) {
		...
		sub128( aSig0, aSig1, term0, term1, out aSig0, out aSig1 );
		shortShift128Left( 0, bSig, 64 - expDiff, out term0, out term1 );
...
static floatx80 floatx80_sqrt( floatx80 a )
{
	...
	shift128Right( aSig0, 0, 2 + ( aExp & 1 ), out aSig0, out aSig1 );
...
static int32 float128_to_int32( float128 a )
{
	...
	if ( 0 < shiftCount ) shift64RightJamming( aSig0, shiftCount, out aSig0 );
...
static int32 float128_to_int32_round_to_zero( float128 a )
{
	...
	if ( 0x401E < aExp ) {
		...
		if ( aExp || aSig0 ) float_exception_flags |= float_flag.float_flag_inexact;
		return 0;
	}
...
static int64 float128_to_int64( float128 a )
{
	...
	if ( shiftCount <= 0 ) {
		...
		shortShift128Left( aSig0, aSig1, - shiftCount, out aSig0, out aSig1 );
	}
	else {
		shift64ExtraRightJamming( aSig0, aSig1, shiftCount, out aSig0, out aSig1 );
	}
	...
	if ( 0 < shiftCount ) {
		if ( 0x403E <= aExp ) {
			...
			if (    ( a.high == LIT64( 0xC03E000000000000 ) )
				...
				if ( aSig1 ) float_exception_flags |= float_flag.float_flag_inexact;
			}
			else {
				float_raise( float_flag.float_flag_invalid );
...
static float128 addFloat128Sigs( float128 a, float128 b, flag zSign )
{
	...
	if ( 0 < expDiff ) {
		...
		shift128ExtraRightJamming(
			bSig0, bSig1, 0, expDiff, out bSig0, out bSig1, out zSig2 );
		zExp = aExp;
	}
	else if ( expDiff < 0 ) {
		if ( bExp == 0x7FFF ) {
		...
		shift128ExtraRightJamming(
			aSig0, aSig1, 0, - expDiff, out aSig0, out aSig1, out zSig2 );
		zExp = bExp;
	}
	else {
		if ( aExp == 0x7FFF ) {
...
static float128 subFloat128Sigs( float128 a, float128 b, flag zSign )
{
	...
	shift128RightJamming( aSig0, aSig1, - expDiff, out aSig0, out aSig1 );
	...
	shift128RightJamming( bSig0, bSig1, expDiff, out bSig0, out bSig1 );
	aSig0 |= LIT64( 0x4000000000000000 );
	aBigger:
...
//    bSign = extractFloat128Sign( b );
	...
	if ( q ) sub128( aSig0, aSig1, bSig0, bSig1, out aSig0, out aSig1 );
	expDiff -= 64;
	while ( 0 < expDiff ) {
	...
	if ( -64 < expDiff ) {
		...
		expDiff += 52;
		if ( expDiff < 0 ) {
			shift128Right( aSig0, aSig1, - expDiff, out aSig0, out aSig1 );
		}
		else {
			shortShift128Left( aSig0, aSig1, expDiff, out aSig0, out aSig1 );
		}
		mul128By64To192( bSig0, bSig1, q, out term0, out term1, out term2 );
	...
	add128(
		aSig0, aSig1, alternateASig0, alternateASig1, (bits64 *)out sigMean0, out sigMean1 );
	if (    ( sigMean0 < 0 )
...
static float128 float128_sqrt( float128 a )
{
	...
	shortShift128Left( aSig0, aSig1, 13 - ( aExp & 1 ), out aSig0, out aSig1 );
	zSig0 = estimateDiv128To64( aSig0, aSig1, zSig0<<32 ) + ( zSig0<<30 );
	doubleZSig0 = zSig0<<1;
	mul64To128( zSig0, zSig0, out term0, out term1 );
	sub128( aSig0, aSig1, term0, term1, out rem0, out rem1 );
